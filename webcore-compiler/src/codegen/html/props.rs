//! Prop substitution helpers for component view rendering.
...
pub(super) fn substitute_props(
    ...
    let combined: std::collections::HashMap<&str, (bool, &str)> = static_props
        ...
        .chain(dynamic_props.iter().map(|(k, v)| (k.as_str(), (false, v.as_str()))))
        .collect();
    elements
        .iter()
...
pub(super) fn elements_combined(
    ...
    elements.iter().map(|e| substitute_props_elem_combined(e, combined)).collect()
}
