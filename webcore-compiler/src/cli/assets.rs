//! Asset handling: content-hash fingerprinting, file copying, HTML/CSS patching.

use super::build::walk_files;
...
pub(crate) fn fingerprint_images(
    ...
    fn walk(dir: &Path, assets_dir: &Path, map: &mut HashMap<String, String>) -> Result<(), String> {
        ...
        for entry in entries.flatten() {
            ...
            fs::copy(&path, &dst)
                .map_err(|e| format!("Failed to copy {} → {}: {e}", path.display(), dst.display()))?;
...
pub(crate) fn patch_html_files(dir: &Path, js_src: &str) {
